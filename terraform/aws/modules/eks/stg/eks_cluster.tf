resource "aws_eks_cluster" "hk-algo-stg-eks-cluster" {
  ...
  vpc_config {
    ...
    public_access_cidrs     = ["113.28.102.128/26", "18.167.224.139/32", "183.14.0.0/16", "42.200.69.243/32"]
    subnet_ids              = [var.subnet_id_1b, var.subnet_id_1a]
  }

  depends_on = [
    aws_eks_cluster.hk-algo-stg-eks-cluster,
  ]
